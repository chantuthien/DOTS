...
public class ExecuteAuthoring : MonoBehaviour
{
    ...
    class Baker : Baker<ExecuteAuthoring>
    {
        public override void Bake(ExecuteAuthoring authoring)
        {
            ...
            if (authoring.TurretRotation)
                AddComponent<TurretRotaion>(entity);
            if (authoring.TankMovement)
                AddComponent<TankMovement>(entity);
            if (authoring.TurretShooting)
                AddComponent<TurretShooting>(entity);
            if (authoring.CannonBall)
                AddComponent<CannonBall>(entity);
            if (authoring.Camera)
                AddComponent<Camera>(entity);
            if (authoring.SafeZone)
                AddComponent<SafeZone>(entity);
            if (authoring.TankSpawning)
                AddComponent<TankSpawning>(entity);
        }
    }
}

public struct TurretRotaion : IComponentData { }
